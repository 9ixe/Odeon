...
public sealed partial class CompositeTrackPicker : UserControl
{
    ...
    {
        get => (bool)GetValue(ShowVideoSectionProperty);
        set => SetValue(ShowVideoSectionProperty, value);
    }

    public static readonly DependencyProperty ShowVideoSectionProperty =
        DependencyProperty.Register(nameof(ShowVideoSection), typeof(bool), typeof(CompositeTrackPicker), new PropertyMetadata(true));

    public bool ShowAudioSection
    {
        get => (bool)GetValue(ShowAudioSectionProperty);
        set => SetValue(ShowAudioSectionProperty, value);
    }

    public static readonly DependencyProperty ShowAudioSectionProperty =
        DependencyProperty.Register(nameof(ShowAudioSection), typeof(bool), typeof(CompositeTrackPicker), new PropertyMetadata(true));

    public bool ShowSubtitleSection
    {
        get => (bool)GetValue(ShowSubtitleSectionProperty);
        set => SetValue(ShowSubtitleSectionProperty, value);
    }
    ...
    private void RebuildAudioDisplayList()
    {
        ...
        if (AudioDisplayList.SequenceEqual(newList)) return;
    ...
    private void RebuildVideoDisplayList()
    {
        ...
        // Avoid clearing and repopulating the existing ObservableCollection to prevent unexpected SelectedIndex change.
        if (VideoDisplayList.SequenceEqual(newList)) return;
