...
public partial class MediaViewModel : ObservableRecipient
{
    ...
    public TimeSpan? OverrideResumePosition { get; set; }
    ...
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(NameWithoutExtension))]
        ...
        : this(file, new MediaInfo(FilesHelpers.GetMediaTypeForFile(file)), playerContext, playerService)
    {
        ...
        _altCaption = Path.GetFileNameWithoutExtension(file.Name);
    }
    ...
    {
        Source = file;
        AltCaption = Path.GetFileNameWithoutExtension(file.Name);
    }
