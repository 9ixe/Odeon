    ...
    IRecipient<PropertyChangedMessage<IMediaPlayer?>>
{
    ...
    public void OnSeekBarValueChanged(object sender, RangeBaseValueChangedEventArgs args)
    {
        ...
        if (IsSeekable && MediaPlayer != null && Math.Abs(Time - args.NewValue) > 10)
        {
            Time = args.NewValue;
            ...
            {
                SetPlayerPosition(newPosition, false);
            }
        }
    ...
    {
        if (media.OverrideResumePosition.HasValue)
        {
            TimeSpan overridePos = media.OverrideResumePosition.Value;
            media.OverrideResumePosition = null;

            if (media.IsPlaying ?? false)
            {
                _dispatcherQueue.TryEnqueue(DispatcherQueuePriority.Low, () => SetPlayerPosition(overridePos, false));
                _lastTrackedPosition = TimeSpan.Zero;
            }
            else
            {
                _lastTrackedPosition = overridePos;
            }
            return;
        }
    ...
    private void SetPlayerPosition(TimeSpan position, bool debounce)
    {
        ...
        if (debounce)
        {
            _seekTimer.Debounce(() =>
            {
                if (MediaPlayer != null && IsSeekable)
                {
