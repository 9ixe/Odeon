...
namespace Odeon.Core.Playback
{
    public sealed class VlcMediaPlayer : IMediaPlayer
    {
        ...
        private void VlcPlayer_ESSelected(object sender, MediaPlayerESSelectedEventArgs e)
        {
            ...
            // VLC sometimes auto selects a subtitle track — sync app UI state to match.
            // NOTE: Do NOT call SetMediaPlayer here. SetMediaPlayer triggers extraction;
            // calling it from ESSelected creates a re-entrant loop (our slave AddSlave
            // fires ESSelected, which cancels and restarts extraction, which adds another
            // slave, which fires ESSelected again, ad infinitum).
            int spu = e.Id;
            if (spu < 0) return;
        ...
        private void VlcPlayer_ESAdded(object sender, MediaPlayerESAddedEventArgs e)
        {
            ...
            {
                int spuId = e.Id;
                // Update the lazy track's VlcSpu so SelectVlcSpu can find it when
                // ESSelected fires immediately after this.
                PlaybackItem.SubtitleTracks.NotifyTrackAdded(spuId);
            }
        }
        ...
        private void VlcPlayer_Playing(object sender, EventArgs e)
        {
            ...
            // Trigger MKV subtitle extraction exactly once per media item.
            // Previously done from ESSelected, which caused a re-entrant loop:
            // AddSlave fires ESSelected → SetMediaPlayer restarts extraction → another AddSlave.
            PlaybackItem?.SubtitleTracks.SetMediaPlayer(this);
        }
